...
这的确可行。这个课时我会为你介绍一个更加==强大的 HTML 解析库：pyquery==。利用它，我们可以直接解析 DOM 节点的结构，并通过 DOM 节点的一些属性快速进行内容提取。
...
我们先用一个实例来感受一下 pyquery 的 `CSS 选择器`的用法：
...
==查找子节点==需要用到 `find` 方法，传入的参数是 CSS 选择器，我们还是以上面的 HTML 为例：
...
`find` 的查找范围是==节点的所有子孙节点==，而如果我们只想查找==子节点==，那可以用 `children` 方法：
...
如果要筛选所有子节点中符合条件的节点，比如想筛选出子节点中 class 为 active 的节点，可以==向 children 方法传入 CSS 选择器== .active，代码如下：
...
但是如果你想==获取某个祖先节点==，该怎么办呢？我们可以用 `parents` 方法：
...
pyquery 提供了一系列方法来对节点进行动态修改，比如==为某个节点添加一个 class==，移除某个节点等，这些操作有时会为提取信息带来极大的便利。
